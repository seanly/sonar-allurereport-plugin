...
An Allure Report plugin for SonarQube compatible with SonarQube 9.x.
...
The JAR will be deployed to `target/sonar-allurereport-plugin-VERSION.jar`. Copy this to your SonarQube's `extensions/plugins/` directory, and re-start SonarQube.
...
* The name of each entry file must correspond to the `page_id` of the registered page (see `src/main/java/org/sonarsource/plugins/allurereport/web/AllureReportPage.java` and compare with the entry points in `conf/webpack/webpack.config.js`).
...
The pages are registered in `src/main/java/org/sonarsource/plugins/allurereport/web/AllureReportPage.java`, and their respective front-end source code is located in `src/main/js/`. These examples use different stacks to demonstrate different possibilities:
...
* **Localization (`window.t()` and `window.tp()`)**  
  Localizable UI strings are defined in `src/main/resources/org/sonar/l10n/allurereport.properties`. They are loaded at startup time, and can used by the global `t()` and `tp()` functions. See `src/main/js/admin_page/components/InstanceStatisticsApp.js` and `src/main/js/portfolio_page/components/VersionsMeasuresHistoryApp.js` for some examples. 
