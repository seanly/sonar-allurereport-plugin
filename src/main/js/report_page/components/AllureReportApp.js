...
export function findAllureReport(options) {
  var request = {
    component : options.component.key,
    metricKeys : "allure-report"
  };
  ...
  return getJSON("/api/measures/component", request).then(function(response) {
    var report = response.component.measures.find((measure) => {
      return measure.metric === "allure-report";
    });
    if (typeof report  === "undefined") {
      return "<center><h2>No HTML-Report found. Please check property sonar.allureReport.htmlReportPath</h2></center>";
    } else {
      return report.value;
    }
  });
...
export default class AllureReportApp extends React.PureComponent {
  constructor() {
    super();
    this.state = {
      loading: true,
  ...
  componentDidMount() {
    ...
    findAllureReport(this.props.options).then((data) => {
      this.setState({
        loading: false,
        data
      });
  ...
  render() {
    ...
    if (this.state.data.startsWith("http")) {
        return (<div className="page allure-report-container" >
                  <iframe classsandbox="allow-scripts allow-same-origin" height={this.state.height} src={this.state.data} style={{border: "none"}} />
                </div>);
    } else {
        return (<div className="page allure-report-container" >
